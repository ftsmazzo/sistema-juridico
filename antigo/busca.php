<? 
  ...
  $title = "Pesquisar prazos com a(s) palavra(s): \"".$busca."\"";
      ...
      <div class="content">
        <div class="row">
          <div class="col-xs-12">
            ...
            <div id="prazos">
              <?
                $prazos = $conexao->query("SELECT p.*, CONCAT(u.nome, ' ', u.sobrenome) as usuario FROM prazos p LEFT JOIN usuarios u ON p.status=u.id WHERE p.observacao LIKE '%".$busca."%' OR p.conteudo LIKE '%".$busca."%' OR p.prazo LIKE '%".$busca."%' ORDER BY FIELD(tipo,'administrativo','civil','trabalhista')");
              ...
              <div class="col-xs-12 mix no-padding <?=$classe." ".$dadosP['tipo'];?>">
                <div class="widget <?=$classe." ".$dadosP['tipo'];?>">
                  <div class="widget-header">
                    <h2 class="col-xs-11"><strong><?=$dadosP['prazo'];?> - <?=formataDataBr($dadosP['data']);?><br/><?=$cumprido;?></strong></h2>
