<? 
  ...
  if(isset($_POST['datainicial']) && !empty($_POST['datainicial']) && dataValida(formataDataSQL($_POST['datainicial'])) && isset($_POST['datafinal']) && !empty($_POST['datafinal']) && dataValida(formataDataSQL($_POST['datafinal'])) && isset($_POST['advogado']) && !empty($_POST['advogado'])) {
    $advogado    = $conexao->real_escape_string($_POST['advogado']);
    $dataInicial = $conexao->real_escape_string(formataDataSQL($_POST['datainicial']));
    $dataFinal   = $conexao->real_escape_string(formataDataSQL($_POST['datafinal']));
        ...
        <div class="row">
          <div class="col-xs-12">
            <h1 class="text-center">
              Relatório do(a) advogado(a) <?=$nome." ".$sobrenome;?> entre as datas de <?=formataDataBr($dataInicial);?> até <?=formataDataBr($dataFinal);?>
              <a href="javascript: w=window.open('adm-print-relatorios.php?advogado=<?=$advogado;?>&datainicial=<?=$dataInicial;?>&datafinal=<?=$dataFinal;?>'); w.print();" class="pull-right" title="Versão para impressão"><i class="fa fa-print"></i></a>
            </h1>
            <br/>
            ...
            <ul class="col-xs-12 text-center filtroMixItUp">
              <?
                $datasPrazos = $conexao->query("SELECT p.*, CONCAT(u.nome, ' ', u.sobrenome) as usuario FROM prazos p LEFT JOIN usuarios u ON p.status=u.id LEFT JOIN prazosUsuarios pu ON p.id=pu.idPrazo WHERE data>='".$dataInicial."' AND data<='".$dataFinal."' AND (status='".$advogado."' OR pu.idUsuario='".$advogado."') GROUP BY data ORDER BY data");
              ...
              <div class="col-xs-12 mix no-padding <?=$classe." ".$dadosP['tipo']." ".$dadosP['data'];?>">
                <div class="widget <?=$classe." ".$dadosP['tipo']." ".$dadosP['data'];?>">
                  <div class="widget-header">
                    <h2><strong><?=$dadosP['prazo'];?><br/><?=$cumprido;?></strong></h2>
