...
| `WEBHOOK_N8N_TAREFAS_INTERNAS_URL` | Não (tarefas internas / WhatsApp) | `https://seu-n8n.com/webhook/tarefas-internas` | Webhook único para **tarefas internas** do escritório. O sistema envia POST JSON com `event` (`criada` \| `cumprida` \| `alerta_d3` \| `cobranca`), `msg`, `numeroEnvio` (E.164 BR), `tarefaInternaId`, `prazoId`, `destinatarioUsuarioId`. No N8N, use **Switch** por `event` e o nó Evolution com o número do **escritório**. Alerta **D−3 dias úteis** roda só entre **9h e 14h** (`America/Sao_Paulo`). Ver `docs/TAREFAS_INTERNAS_CHECKLIST.md`. |
| `WEBHOOK_N8N_SUGESTAO_CHECKLIST` | Não (Sugerir passos com IA) | `https://seu-n8n.com/webhook/sugestao-checklist` | URL do **webhook no N8N** para sugerir itens de checklist do prazo. O sistema envia POST com contexto do prazo (publicação, movimentação, processo quando vinculado). Resposta esperada: `{ "itens": [ { "titulo": "..." }, ... ] }` ou array de strings. Se não configurado, são retornadas sugestões genéricas por tipo de movimentação. |
| `EMAIL_MONITOR_ENCRYPTION_KEY` | Não | 32 ou 64 caracteres hex | Chave para criptografar a senha da conta IMAP. Se **não** definir, a senha ainda funciona (gravada de forma que o sistema use na verificação). Para criptografia forte, defina 32 ou 64 hex (ex.: gere em [random.org/strings](https://www.random.org/strings/) e cole aqui). |
